...
void print_hash(char *msg)
{
    ...
    {
        printf("[%d] %d ,", i, hash_arr[i]); //Printing the index first and then the value at that index after that
    }
    printf("\n");
}
...
void hash_init()
{
    memset(hash_init, 0, sizeof(hash_arr)); //Initializing the hash array to 0 in each of the array's index
}

int hash_insert(int n, int idx)
{
    if(hash_arr[n % HASH_SIZE] == 0) //Checking if the spot in the hash array is empty, as all indices in the array have been initialized to 0 by the initialization function(void hash_init())
    {
...
int hash_find(int n)
{
    ...
    if(hash_arr[n % HASH_SIZE] != 0) //Checks if there is an existing element at the searched location
    {
